...
func main() {
	job := func(log *slog.Log) error {
		if response, err := http.Request("http://"+config.Get("http.address").(string)+"/v1/test/id-01", net_http.MethodGet, map[string][]string{"header-1": {"value-1"}}, "", 3*time.Second, "", "", nil); err != nil {
			return err
		} else {
